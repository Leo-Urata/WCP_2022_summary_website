...
def games():
    ...
    cur1.execute("SELECT cid,uname,message,pdate FROM pychat ORDER BY cid DESC FETCH FIRST 5 ROWS ONLY;")
    chat = cur1.fetchall()
    cur1.close()
    
    cur2 = connection.cursor()
    cur2.execute("select * from games where category_name LIKE '%第１日%';")
    group_day1 = cur2.fetchall()
    cur2.close()
    
    cur3 = connection.cursor()
    cur3.execute("select * from games where category_name LIKE '%第２日%';")
    group_day2 = cur3.fetchall()
    cur3.close()
    
    cur4 = connection.cursor()
    cur4.execute("select * from games where category_name LIKE '%第３日%';")
    group_day3 = cur4.fetchall()
    cur4.close()
    
    cur5 = connection.cursor()
    cur5.execute("select * from games where category_name = 'ラウンド16';")
    round_of_16 = cur5.fetchall()
    cur5.close()
    
    cur6 = connection.cursor()
    cur6.execute("select * from games where category_name = '準々決勝';")
    round_of_8 = cur6.fetchall()
    cur6.close()
    
    cur7 = connection.cursor()
    cur7.execute("select * from games where category_name = '準決勝';")
    semi_finals = cur7.fetchall()
    cur7.close()
    
    cur8 = connection.cursor()
    cur8.execute("select * from games where category_name = '3位決定戦';")
    third_place_match = cur8.fetchall()
    cur8.close()
    
    cur9 = connection.cursor()
    cur9.execute("select * from games where category_name = '決勝';")
    final = cur9.fetchall()
    cur9.close()
    
    return render_template('games.html', title='flask chat', chat=chat, group_day1=group_day1, group_day2=group_day2, group_day3=group_day3, round_of_16=round_of_16, round_of_8=round_of_8, semi_finals=semi_finals, third_place_match=third_place_match, final=final)
...
def details():
    ...
    cur1 = connection.cursor()
    cur1.execute("select name, url from managers where national = %s ;", (contents,))
    mn = cur1.fetchall()
    cur1.close()
    
    cur2 = connection.cursor()
    cur2.execute("select name, Uname, club, url from players where national = %s and Position  = 'GK' order by Uname asc;", (contents,))
    gk = cur2.fetchall()
    cur2.close()
    
    cur3 = connection.cursor()
    cur3.execute("select name, Uname, club, url from players where national = %s and Position  = 'DF';", (contents,))
    df = cur3.fetchall()
    cur3.close()
    
    cur4 = connection.cursor()
    cur4.execute("select name, Uname, club, url from players where national = %s and Position  = 'MF';", (contents,))
    mf = cur4.fetchall()
    cur4.close()
    
    cur5 = connection.cursor()
    cur5.execute("select name, Uname, club, url from players where national = %s and Position  = 'FW';", (contents,))
    fw = cur5.fetchall()
    cur5.close()
